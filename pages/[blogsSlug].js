...
export async function getServerSideProps(context) {
    const post5 = await fetch('https://byldblogs-4rws.onrender.com/api/posts')
    const posts5 = await post5.json()

    const posturl = context.params.blogsSlug;
    const res = await fetch(`https://byldblogs-4rws.onrender.com/api/post/${posturl}`)
    const posts = await res.json()

    const getfeedback = await fetch(`https://byldblogs-4rws.onrender.com/api/posts/feed/${posturl}`)
...
const submitFeedback = async (event) => {
    ...
    xhr.open('POST', 'https://byldblogs-4rws.onrender.com/api/feedback/');
